...
const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <Card title="Welcome to Validate Hub!" className="bg-white"> {/* Changed background to white */}
        <p className="text-lg text-gray-800"> {/* Changed text color to dark gray */}
          Ready to turn your business idea into a validated model? Validate Hub guides you through each step.
        </p>
        <p className="mt-2 text-sm text-gray-800"> {/* Changed text color to dark gray */}
          ...
          <Link to={link.path} key={link.name} className="block hover:shadow-xl transition-shadow duration-200 rounded-xl">
            <Card className={`h-full ${link.bgColor}`}>
              <div className="flex flex-col items-center justify-center p-4 text-center">
                <link.icon className={`h-12 w-12 mb-3 ${link.color}`} />
                <h3 className={`text-xl font-semibold ${link.color}`}>{link.name}</h3>
                <p className="text-sm text-gray-800 mt-1">Navigate to {link.name.toLowerCase()} module.</p>
              </div>
            </Card>
          </Link>
        ))}
      ...
      <Card title="Your Validation Journey">
        <div className="flex items-center space-x-4">
          <ChartBarIcon className="h-10 w-10 text-secondary-DEFAULT" />
          <div>
         ...
         <div className="mt-4 h-32 bg-gray-200 rounded flex items-center justify-center text-gray-500">
